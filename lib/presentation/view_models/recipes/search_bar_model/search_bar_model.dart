...
class SearchBarModel extends StateNotifier<SearchBarModelState> {
  ...
  void init() {
    state = SearchBarModelState.ready(false);
  }

  void expandSearchBar() {
    if (state is SearchBarModelReady) {
      final state = this.state as SearchBarModelReady;
      print("TOGGLE EXPAND TO ${!state.recipeSearchBarExpanded}");
      this.state = SearchBarModelState.ready(!state.recipeSearchBarExpanded);
    }
  }
}
